...
(define (palindrome? word)
  (equal? word (reverse-string word)))

(define (convert-string str)
  (string-downcase 
   (regexp-replace* #px"[^a-zA-Z0-9 ]" str "")))

(define (palindrome-list s)
  (let* ([converted (convert-string s)]
         [words (string-split converted)]
...
(define (check-bt t (min-val -inf.0) (max-val +inf.0))
  (cond
    [(bt-empty? t) #t]
    [(bt-leaf? t)
     (let ([val (bt-leaf-val t)])
       (and (< min-val val) (< val max-val)))]
    [(bt-node? t)
     (let ([val (bt-node-val t)]
           [left (bt-node-left t)]
           [right (bt-node-right t)])
       (and (< min-val val) (< val max-val)
            (check-bt left min-val val)
            (check-bt right val max-val)))]))
...
(define (ml-split lst)
  (define (split-helper lst current-seq result direction)
    (cond
      [(null? lst) 
       (if (null? current-seq)
           (reverse result)
           (reverse (cons (reverse current-seq) result)))]
      [(null? current-seq) 
       (split-helper (cdr lst) (list (car lst)) result 'none)]
      [else
       (let ([prev (car current-seq)]
             [curr (car lst)])
         (cond
           [(eq? direction 'none)
            (if (= curr prev)
                (split-helper (cdr lst) (list curr) (cons (reverse current-seq) result) 'none)
                (split-helper (cdr lst) (cons curr current-seq) result 
                              (if (> curr prev) 'up 'down)))]
           [(eq? direction 'up)
            (if (<= curr prev)
                (split-helper (cdr lst) (list curr) (cons (reverse current-seq) result) 'none)
                (split-helper (cdr lst) (cons curr current-seq) result 'up))]
           [(eq? direction 'down)
            (if (>= curr prev)
                (split-helper (cdr lst) (list curr) (cons (reverse current-seq) result) 'none)
                (split-helper (cdr lst) (cons curr current-seq) result 'down))]))]))
  
  (split-helper lst '() '() 'none))
...
(define (piles-insert ls n)
  (define (insert piles)
    (cond
      [(null? piles) (list (list n))]
      [(<= n (car (car piles))) (cons (cons n (car piles)) (cdr piles))]
      [else (cons (car piles) (insert (cdr piles)))]))
